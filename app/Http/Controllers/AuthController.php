...
class AuthController extends Controller
{
    ...
    public function login(Request $request)
    {
        ...
        foreach ($decryptDatas as $data) {
            // Check if correct match in database
            if ($data['email'] == $request->input("email") && Hash::check($request->input('password'), $data['password']) && $data['email_verified_at'] !== null) {
                // $expirationTime = Carbon::now()->addSeconds(30);
                // Expiration Time 1month
                $userSavingToken = AuthModel::where('id_hash', $data['id_hash']);
                $expirationTime = Carbon::now()->addMinutes(2592000);
                $newToken = JWTAuth::claims(['exp' => $expirationTime->timestamp])->fromUser($userSavingToken);
                if (!$newToken) {
